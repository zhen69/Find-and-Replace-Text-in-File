...
int run_using_system(char *test_name, char *args) {
    char *executable = "./bin/FindReplace";
    assert(access(executable, F_OK) == 0);

    char cmd[500];
    sprintf(test_log_outfile, "%s/%s.log", TEST_OUTPUT_DIR, test_name);
#if defined(__linux__)
    sprintf(cmd, "ulimit -f 300; ulimit -t 5; valgrind -s --leak-check=full --show-leak-kinds=all --track-origins=yes --trace-children=yes --error-exitcode=37 ./bin/FindReplace %s >> %s 2>&1",
...
int run_using_system_no_valgrind(char *test_name, char *args) {
    char executable[100];
    sprintf(executable, "./bin/FindReplace");
    assert(access(executable, F_OK) == 0);

    char cmd[500];
    sprintf(test_log_outfile, "%s/%s.log", TEST_OUTPUT_DIR, test_name);
    sprintf(cmd, "ulimit -f 300; ulimit -t 5; ./bin/FindReplace %s >> %s 2>&1", args, test_log_outfile);
    return system(cmd);
}
...
/*Test prgram using only -s, -r, and -l*/
...
Test(base_output, simple_search11, .description="Perform a simple replacement over a specific range.") {
    char *test_name = "simple_search11";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-p nasinanisna ansansniasni -a -s the -r \"WOLFIE LAND\" -l 1,2,3,4,5,6,7 %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(base_output, simple_search12, .description="Search_text = Replacement_text") {
    char *test_name = "simple_search12";
    prep_files("turing.txt", test_name);    
    sprintf(args, "-l 1,1000 -s the -j iansainsna ansanksnsn -r the %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(base_output, simple_search13, .description="Line Numeber > size of file") {
    char *test_name = "simple_search13";
    prep_files("turing.txt", test_name);    
    sprintf(args, "-r \"Hello World\" -l 1000,1000 -s a %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(base_output, simple_search14, .description="Repalcement is long") {
    char *test_name = "simple_search14";
    prep_files("camelSpace.txt", test_name);    
    sprintf(args, "-a -r \"Hello World Nice?\" -s o %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}


//Test program using -w
...
Test(base_output, wildcard_search25, .description="Perform a wildcard suffix replacement.") {
    char *test_name = "wildcard_search25";
    prep_files("random.txt", test_name);    
    sprintf(args, "-a kasnasnka -s *ax -r \"Gamma???12345\" -w %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(base_output, wildcard_search26, .description="Perform a wildcard prefix replacement.") {
    char *test_name = "wildcard_search26";
    prep_files("random.txt", test_name);    
    sprintf(args, "-s ax* -l 1,200 -p -j -k -r \"()Gamma???12345\" -w %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(base_output, wildcard_search27, .description="Test *ha in hahahaha") {
    char *test_name = "wildcard_search27";
    prep_files("camelSpace.txt", test_name);    
    sprintf(args, "-w -1 1,2 -s *ha -r \"Hulk Smash\" %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(base_output, wildcard_search28, .description="Test *ha in ha.txt") {
    char *test_name = "wildcard_search28";
    prep_files("ha.txt", test_name);    
    sprintf(args, "-r Garfield -w -l 1,2 -s *ha %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(base_output, wildcard_search29, .description="Test ha* in ha.txt") {
    char *test_name = "wildcard_search29";
    prep_files("ha.txt", test_name);    
    sprintf(args, "-r Garfield -w -s ha* -l 1,3 %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}
...
Test(base_valgrind, simple_search11) {
    char *test_name = "simple_search11";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-p nasinanisna ansansniasni -a -s the -r \"WOLFIE LAND\" -l 1,2,3,4,5,6,7 %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, simple_search12) {
    char *test_name = "simple_search12";
    prep_files("turing.txt", test_name);    
    sprintf(args, "-l 1,1000 -s the -j iansainsna ansanksnsn -r the %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, simple_search13) {
    char *test_name = "simple_search13";
    prep_files("turing.txt", test_name);    
    sprintf(args, "-r \"Hello World\" -l 1000,1000 -s a %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, simple_search14) {
    char *test_name = "simple_search14";
    prep_files("camelSpace.txt", test_name);    
    sprintf(args, "-a -r \"Hello World Nice?\" -s o %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, wildcard_search25) {
    char *test_name = "wildcard_search25";
    prep_files("random.txt", test_name);    
    sprintf(args, "-a kasnasnka -s *ax -r \"Gamma???12345\" -w %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, wildcard_search26) {
    char *test_name = "wildcard_search26";
    prep_files("random.txt", test_name);    
    sprintf(args, "-s ax* -l 1,200 -p -j -k -r \"()Gamma???12345\" -w %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, wildcard_search27) {
    char *test_name = "wildcard_search27";
    prep_files("camelSpace.txt", test_name);    
    sprintf(args, "-w -1 1,2 -s *ha -r \"Hulk Smash\" %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, wildcard_search28) {
    char *test_name = "wildcard_search28";
    prep_files("ha.txt", test_name);    
    sprintf(args, "-r Garfield -w -l 1,2 -s *ha %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_valgrind, wildcard_search29) {
    char *test_name = "wildcard_search29";
    prep_files("ha.txt", test_name);    
    sprintf(args, "-r Garfield -w -s ha* -l 1,3 %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    expect_no_valgrind_errors(run_using_system(test_name, args));
...
Test(base_invalid_args, args_missing02, .description="Argument missing.") {
    char *test_name = "args_missing02";
    sprintf(args, "-s stony %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, MISSING_ARGUMENT);
}

Test(base_invalid_args, args_missing03, .description="Argument missing.") {
...
Test(base_invalid_args, duplicate_argument02, .description="Duplicate argument.") {
    char *test_name = "duplicate_argument02";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-s the -w -r end -r end -s hello -l 10,15 %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, DUPLICATE_ARGUMENT);
}

Test(base_invalid_args, duplicate_argument03, .description="Duplicate argument.") {
    char *test_name = "duplicate_argument03";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-s the -r end -l 10,15 -s hello %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, DUPLICATE_ARGUMENT);
}

Test(base_invalid_args, duplicate_argument04, .description="Duplicate argument.") {
    char *test_name = "duplicate_argument04";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-s the -r end -l 10,15 -r what %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, DUPLICATE_ARGUMENT);
}

Test(base_invalid_args, duplicate_argument05, .description="Duplicate argument.") {
    char *test_name = "duplicate_argument05";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-w -r end -s *the -l 10,15 -w %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, DUPLICATE_ARGUMENT);
}
...
Test(base_invalid_args, s_argument_missing04, .description="-s argument is missing") {
    char *test_name = "s_argument_missing04";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-s -r end -l 10,15 -w %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, S_ARGUMENT_MISSING);
}


//Missing -r flag or -r argument
...
Test(base_invalid_args, r_argument_missing04, .description="-r argument is missing") {
    char *test_name = "r_argument_missing04";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-s the -a aksnasakns askkasksam -r -l 10,15 %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, R_ARGUMENT_MISSING);
}


//Missing -l argument or invalid -l argument
...
Test(base_invalid_args, wildcard_invalid03, .description="Search text is just *") {
    char *test_name = "wildcard_invalid03";
    prep_files("unix.txt", test_name);    
    sprintf(args, "-s * -r BOT -w %s/%s.in.txt %s/%s.out.txt", TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    int status = run_using_system_no_valgrind(test_name, args);
    expect_error_exit(status, WILDCARD_INVALID);
}